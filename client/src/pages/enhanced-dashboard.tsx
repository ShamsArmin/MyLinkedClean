  ...
  const reorderLinksMutation = useMutation({
    mutationFn: async ({ id, direction }: { id: number; direction: 'up' | 'down' }) => {
      ...
      const linkScores = links.map((link, index) => {
        if (index === linkIndex) return { id: link.id, score: links[newIndex].aiScore || 0 };
        if (index === newIndex) return { id: link.id, score: links[linkIndex].aiScore || 0 };
        return { id: link.id, score: link.aiScore || 0 };
      });
