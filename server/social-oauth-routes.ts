...
// Debug routes for OAuth callbacks
socialOAuthRouter.get("/auth/:platform/callback", async (req, res) => {
  const { platform } = req.params;
  console.log(`${platform.toUpperCase()} OAuth callback received:`);
  ...
  res.json({
    message: `${platform.toUpperCase()} OAuth callback received`,
    platform: platform,
    ...
    instructions: `Use this information to debug the ${platform} OAuth flow. The 'code' parameter is needed to exchange for an access token.`
  });
});
...
async function getUserProfile(platform: string, accessToken: string) {
  ...
  if (platform === 'tiktok') {
    return {
      id: data.data?.open_id || data.open_id,
      username: data.data?.display_name || data.display_name
    };
  }
...
async function syncUserPosts(userId: number, platform: string, accessToken: string) {
  try {
    ...
    await storage.updateSocialConnectionSyncTime(userId, platform);
  } catch (error) {
...
async function fetchRecentPosts(platform: string, accessToken: string) {
  const endpoints = {
    instagram: 'https://graph.instagram.com/me/media?fields=id,media_type,media_url,thumbnail_url,caption,timestamp,permalink&limit=10',
    facebook: 'https://graph.facebook.com/me/posts?fields=id,message,created_time,full_picture&limit=10',
    linkedin: 'https://api.linkedin.com/v2/shares?q=owners&owners=urn:li:person:${userId}&count=10',
    twitter: 'https://api.twitter.com/2/users/me/tweets?tweet.fields=created_at,text,attachments&expansions=attachments.media_keys&media.fields=url,preview_image_url&max_results=10'
  };

  const response = await fetch(endpoints[platform as keyof typeof endpoints], {
    headers: {
  ...
  switch (platform) {
    ...
    case 'twitter':
      return data.data?.map((post: any) => ({
        ...
        thumbnail: post.attachments?.media_keys?.[0] ? data.includes?.media?.find((m: any) => m.media_key === post.attachments.media_keys[0])?.preview_image_url : null,
        caption: post.text,
